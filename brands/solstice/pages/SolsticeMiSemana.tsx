...
export default function SolsticeMiSemana() {
  ...
  if (noReg || !reg) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-4" style={{ background: C.bg }}>
      <div className="w-16 h-16 rounded-full border-2 flex items-center justify-center" style={{ borderColor: `${C.red}30` }}>
        <Calendar size={24} style={{ color: `${C.red}60` }} />
      </div>
      <div className="text-center space-y-2">
        <h2 className="text-2xl uppercase" style={{ fontFamily: "'Poiret One', sans-serif", letterSpacing: '0.1em' }}>
          Sin reserva activa
        </h2>
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', color: C.cream, fontFamily: "'Archivo', sans-serif" }}>
      ...
      <div className="px-8 pt-10 pb-6" style={{ borderBottom: `1px solid ${C.gray}15` }}>
        <p className="text-[9px] uppercase font-bold mb-1" style={{ color: C.red, letterSpacing: '0.4em' }}>Mi reserva</p>
      ...
      <div className="px-8 py-8 max-w-4xl space-y-8">
        ...
        {hasRisk && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            className="flex items-start gap-4 px-5 py-4" style={{ background: `${C.red}10`, border: `1px solid ${C.red}35` }}>
            <AlertTriangle size={18} style={{ color: C.red }} className="shrink-0 mt-0.5" />
            <div>
              <p className="text-xs font-black uppercase mb-1" style={{ color: C.red, letterSpacing: '0.15em' }}>
                Cuota{overdue.length > 1 ? 's' : ''} vencida{overdue.length > 1 ? 's' : ''}
              </p>
              <p className="text-[10px] uppercase leading-relaxed" style={{ color: `${C.red}cc`, letterSpacing: '0.1em' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-2 p-8 relative overflow-hidden" style={{ background: C.bgS, border: `1px solid ${C.gray}15` }}>
            <Shield size={100} className="absolute top-4 right-4 opacity-5" style={{ color: C.red }} />
            <div className="relative z-10 space-y-6">
              <div>
                <span className="text-[10px] font-black uppercase px-2 py-1 inline-block mb-3"
                  style={{ border: `1px solid ${C.red}`, color: C.red, letterSpacing: '0.2em' }}>
                  {reg.status === 'active' ? 'Reserva Confirmada' : reg.status === 'reserved' ? 'Pago Inicial Recibido' : reg.status.toUpperCase()}
                </span>
                <h2 className="text-3xl uppercase mb-1" style={{ fontFamily: "'Poiret One', sans-serif" }}>
                  {reg.customer_name}
                </h2>
                <p className="text-xs uppercase" style={{ color: C.gray, letterSpacing: '0.2em' }}>
              ...
              <div className="flex flex-wrap gap-8">
                <div>
                  <p className="text-[8px] uppercase mb-1" style={{ color: C.gray, letterSpacing: '0.25em' }}>Modalidad</p>
                  <p className="text-sm font-bold uppercase">{MODE_LABEL[reg.payment_mode] || reg.payment_mode}</p>
                </div>
                <div>
                  <p className="text-[8px] uppercase mb-1" style={{ color: C.gray, letterSpacing: '0.25em' }}>Pagos completados</p>
                  <p className="text-sm font-bold">{paid.length} / {(reg.schedules?.length || 0) + 1} <span style={{ color: C.gray, fontSize: '10px' }}>(incl. reserva)</span></p>
                </div>
                <div>
                  <p className="text-[8px] uppercase mb-1" style={{ color: C.gray, letterSpacing: '0.25em' }}>Orden</p>
                  <p className="text-sm font-mono">{reg.order_number}</p>
                </div>
              </div>
              {/* Progress bar */}
              <div>
                <div className="flex justify-between text-[9px] uppercase mb-1" style={{ color: C.gray }}>
                  <span>Avance de pago</span>
                  <span style={{ color: C.red }}>{paidPct.toFixed(0)}%</span>
                </div>
                <div className="h-1 w-full rounded-full overflow-hidden" style={{ background: `${C.gray}20` }}>
                  <motion.div className="h-full rounded-full"
                    initial={{ width: 0 }} animate={{ width: `${paidPct}%` }}
                    transition={{ duration: 1 }}
                    style={{ background: C.red }} />
                </div>
                <div className="flex justify-between text-[9px] uppercase mt-1" style={{ color: C.gray }}>
          ...
          {/* Next payment card */}
          {nextDue ? (
            <div className="p-8 flex flex-col justify-between" style={{ background: hasRisk ? `${C.red}15` : C.red, border: hasRisk ? `1px solid ${C.red}` : 'none' }}>
              <div>
                <h3 className="text-xs font-black uppercase tracking-widest mb-3"
                  style={{ color: hasRisk ? C.red : C.cream }}>
                  {hasRisk ? '⚠ Cuota vencida' : 'Próximo pago'}
                </h3>
                <p className="text-5xl font-black mb-2" style={{ color: hasRisk ? C.red : C.cream, fontStretch: '125%' }}>
                  {fmtK(nextDue.amount)}
                </p>
                <p className="text-[10px] uppercase" style={{ color: hasRisk ? `${C.red}90` : `${C.cream}cc`, letterSpacing: '0.2em' }}>
                  {hasRisk ? 'Venció: ' : 'Vence: '}
                  {new Date(nextDue.due_date).toLocaleDateString('es-CO', { month: 'long', day: 'numeric', year: 'numeric' })}
                </p>
              </div>
              <button className="w-full py-4 text-xs font-black tracking-widest uppercase mt-4 transition-all hover:scale-[1.02]"
                style={{ background: hasRisk ? C.red : C.cream, color: hasRisk ? C.cream : C.red }}>
            ...
            <div className="p-8 flex flex-col items-center justify-center gap-4" style={{ background: `${C.green}10`, border: `1px solid ${C.green}30` }}>
              <CheckCircle2 size={40} style={{ color: C.green }} />
              <div className="text-center">
                <p className="text-sm font-black uppercase mb-1" style={{ color: C.green }}>¡Al día!</p>
                <p className="text-[9px] uppercase" style={{ color: C.gray }}>No tienes cuotas pendientes</p>
              </div>
            </div>
          )}
        </div>
              ...
              {reg.schedules.map((sc, idx) => (
                <div key={sc.id}
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <p className="text-sm font-bold">{fmt(sc.amount)} COP</p>
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest"
                    style={{ color: sc.status === 'paid' ? C.green : sc.status === 'overdue' ? C.red : C.gray }}>
        ...
        {/* ── Promotor asignado ── */}
        {reg.seller && (
          <div className="p-6 flex items-center gap-6" style={{ background: C.bgS, border: `1px solid ${C.gray}15` }}>
            <div className="w-12 h-12 rounded-full flex items-center justify-center shrink-0"
              style={{ background: `${C.red}20`, border: `1px solid ${C.red}30` }}>
              <User size={20} style={{ color: C.red }} />
            </div>
            <div className="flex-1">
              <p className="text-[8px] uppercase mb-1" style={{ color: C.gray, letterSpacing: '0.25em' }}>Tu promotor asignado</p>
              <p className="text-sm font-bold uppercase" style={{ letterSpacing: '0.1em' }}>{reg.seller.name || '—'}</p>
              {reg.seller.email && (
            ...
            <code className="text-[9px] px-2 py-1" style={{ background: `${C.gray}15`, color: C.gray }}>
              {reg.seller.ref_code}
            </code>
          </div>
        )}
        ...
        {/* ── Info personal ── */}
        <div className="p-6 space-y-3" style={{ background: C.bgS, border: `1px solid ${C.gray}15` }}>
          <p className="text-[9px] uppercase tracking-widest mb-4" style={{ color: C.gray }}>Mis datos de registro</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {[
              ...
              <div key={label} className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[8px] uppercase" style={{ color: C.gray, letterSpacing: '0.2em' }}>{label}</p>
