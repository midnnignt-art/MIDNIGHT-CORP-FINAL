    ...
    <div style={{ background: C.bg, minHeight: '100vh', color: C.cream, fontFamily: "'Archivo', sans-serif" }}>
      ...
      <div className="px-8 pt-6 space-y-4">
        ...
        <div className="flex items-center gap-1 flex-wrap">
          <span className="text-[9px] uppercase tracking-widest mr-2" style={{ color: C.gray }}>Día</span>
          {DAYS.map(d => (
            <button key={d} onClick={() => setDay(d)}
              className="w-9 h-9 text-xs font-black transition-all"
              style={{
                background: day === d ? C.red : `${C.gray}12`,
                color: day === d ? C.cream : C.gray,
                border: day === d ? 'none' : `1px solid ${C.gray}20`,
              }}>
              {d}
            </button>
          ))}
        ...
        <div className="grid grid-cols-3 gap-3 max-w-sm">
          {[
            ...
            <div key={label} className="p-3" style={{ background: C.bgS, border: `1px solid ${C.gray}15` }}>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 flex-1 min-w-52 px-3 py-2"
            ...
            />
            {search && (
              <button onClick={() => setSearch('')} style={{ color: C.gray }}>
                <X size={12} />
              </button>
            )}
          </div>
          ...
          <button onClick={() => setScanning(v => !v)}
            ...
            style={{
              background: scanning ? `${C.red}20` : C.bgS,
              border: `1px solid ${scanning ? C.red : C.gray + '20'}`,
      ...
      <AnimatePresence>
        {scanning && (
          <motion.div
            ...
            <div className="max-w-sm p-4 space-y-3" style={{ background: C.bgS, border: `1px solid ${C.red}30` }}>
      ...
      <div className="px-8 pt-5 pb-20 max-w-4xl">
        {filtered.length === 0 ? (
          <div className="py-20 text-center" style={{ color: C.gray }}>
            ...
            <p className="text-xs uppercase tracking-widest">Sin registros</p>
          </div>
        ) : (
              ...
              return (
                <div key={reg.id}
                  ...
                  style={{
                    ...
                    background: done ? `${C.green}06` : 'transparent',
                  }}>
                  ...
                  <div className="col-span-3 flex justify-end">
                    {done ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[9px] uppercase font-black" style={{ color: C.green }}>
                          Ingresó
                        </span>
                      </div>
                    ) : (
                      <button
                        ...
                        onMouseEnter={e => { if (!busy) (e.currentTarget as HTMLButtonElement).style.background = `${C.red}12`; }}
        ...
        {stats.total > 0 && (
          <div className="mt-4 flex items-center gap-3 px-4 py-3"
            ...
            <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: `${C.gray}20` }}>
              <div className="h-full rounded-full transition-all"
                style={{ width: `${(stats.done / stats.total) * 100}%`, background: C.green }} />
            </div>
            <span className="text-[10px] font-black shrink-0" style={{ color: C.green }}>
